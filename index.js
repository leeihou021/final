        ...
        const result = await db.files.create(
          {
            ...
            type: req.body.type,
            popupActivo: req.body.popupActivo,
            name
          }
        )
        res.status(200)
...
app.get('/new/:name', async (req, res) => {
  if (process.env.FTP === 'false') {
    const path = process.cwd() + '/image/' + req.params.name
    const exists = fs.existsSync(path)
    if (exists) {
      res.status(200)
      res.sendFile(path)
...
app.patch('/new/:id', async (req, res) => {
  ...
  try {
    // 檢查相片擁有者是不是本人
    let result = await db.news.findById(req.params.id)
    if (result.user !== req.session.user) {
      res.status(403)
      res.send({ success: false, message: '無權限' })
      return
    ...
    // findByIdAndUpdate 預設回傳的是更新前的資料
    // 設定 new true 後會變成回傳新的資料
    result = await db.news.findByIdAndUpdate(req.params.id, req.body, { new: true })
    res.status(200)
    res.send({ success: true, message: '', result })
  } catch (error) {
    if (error.name === 'CastError') {
...
app.delete('/new/:id', async (req, res) => {
  ...
  try {
    ...
    // findByIdAndUpdate 預設回傳的是更新前的資料
    // 設定 new true 後會變成回傳新的資料
    result = await db.news.findByIdAndDelete(req.params.id)
    if (result === null) {
      res.status(404)
      res.send({ success: true, message: '找不到資料' })
    } else {
